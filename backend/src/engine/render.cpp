...
void dispose_mesh(Mesh* mesh) {
    ...
    mesh->selected.clear();
    mesh->selected_vertices_indices.clear();
    printf("dispose mesh\n");
}
...
void load_mesh(Model* model, u32 i, const aiMesh* paiMesh) {
    ...
    std::vector<u32> selected_vertices_indices; //contains only selected vertices' indices
    ...
    for(u32 i = 0; i < paiMesh->mNumVertices; ++i) {
        ...
        selected_vertices_indices.push_back(i);
    }
    ...
    model->meshes[i].selected_vertices_indices = selected_vertices_indices;
